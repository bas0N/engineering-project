...
public class AdminServiceTest {
    ...
    void getUserById_Fail_UserNotFound() {
        ...
        assertEquals(userId.toString(), exception.getFieldValue());
    }

    @Test
    void getAllUsers_Success() {
        // Arrange
        User user1 = new User(1L, "uuid1", "test1@example.com", null, Role.USER, false, true);
        User user2 = new User(2L, "uuid2", "test2@example.com", null, Role.USER, false, true);
    ...
    @Test
    void getAllUsers_Fail_InvalidPageSize() {
        // Arrange
        int page = -1;
        int size = 0;
        ...
        ApiRequestException exception = assertThrows(ApiRequestException.class, () ->
                adminService.getAllUsers(page, size, null, null, null, null, null)
        );

        assertEquals("Invalid parameters for fetching users", exception.getMessage());
        assertInstanceOf(IllegalArgumentException.class, exception.getCause());
        assertEquals("Page index must be >= 0 and size must be > 0", exception.getCause().getMessage());
    }
