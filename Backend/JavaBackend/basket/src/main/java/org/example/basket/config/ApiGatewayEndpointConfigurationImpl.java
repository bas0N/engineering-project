...
public class   ApiGatewayEndpointConfigurationImpl implements ApiGatewayEndpointConfiguration {
    ...
    @Override
    public void initMap() {
        endpointList.add(new Endpoint("/api/v1/basket", HttpMethod.GET, Role.USER));
        endpointList.add(new Endpoint("/api/v1/basket", HttpMethod.POST, Role.USER));
        endpointList.add(new Endpoint("/api/v1/basket", HttpMethod.DELETE, Role.USER));
    }
