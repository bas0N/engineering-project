...
def add_product():
    ...
    try:
        response_data = product_service.add_product_embedding(data)
    except Exception as e:
        # Log the error
        logging.error("Error processing data with service A: %s", str(e))
...
def get_products():
    ...
    try:
        ...
        logging.error("Invalid type for 'number_of_products': expected integer")
        return jsonify({
            "error": "Invalid request",
        ...
        logging.error("Error retrieving similar products: %s", str(e))
        return jsonify({
            "error": "Processing error",
            "message": "An error occurred while processing your request. Please try again later."
        }), 500  # HTTP 500 Internal Server Error
